...
#' confirmation.
#'
#' @inheritParams pactaverse_deps
#' @export
#' @examples
#' \dontrun{
#' pactaverse_update()
#' }
pactaverse_update <- function(recursive = FALSE, repos = getOption("repos")) {

  deps <- pactaverse_deps(recursive, repos)
  behind <- dplyr::filter(deps, behind)

  if (nrow(behind) == 0) {
    cli::cat_line("All pactaverse packages up-to-date")
    return(invisible())
  }
...
#' well as all pactaverse packages. It's primarily designed to help you get
#' a quick idea of what's going on when you're helping someone else debug
#' a problem.
#'
#' @export
pactaverse_sitrep <- function() {
...
#' @export
pactaverse_deps <- function(recursive = FALSE, repos = getOption("repos")) {
  pkgs <- utils::available.packages(repos = repos)
  deps <- tools::package_dependencies("pactaverse", pkgs, recursive = recursive)
