...
pactaverse_conflicts <- function(only = NULL) {
  ...
  tidy_names <- paste0("package:", pactaverse_packages())
  conflicts <- purrr::keep(conflicts, ~ any(.x %in% tidy_names))

  conflict_funs <- purrr::imap(conflicts, confirm_conflict)
  conflict_funs <- purrr::compact(conflict_funs)

  structure(conflict_funs, class = "pactaverse_conflicts")
}

pactaverse_conflict_message <- function(x) {
  header <- cli::rule(
    left = cli::style_bold("Conflicts"),
    right = "pactaverse_conflicts()"
  )
...
#' @export
print.pactaverse_conflicts <- function(x, ..., startup = FALSE) {
  cli::cat_line(pactaverse_conflict_message(x))
  invisible(x)
}
